...
'umath'
'cal_PV'
'tl', 'np', 'opt', 'plt', 'interp1d', 'pd', 'extrapolate', 'cal_dV', 
'plot_dV_fux', 'R', 'xlsx', 'W', 'planets', 'unpack_par', 'fo2_cal_single', 
'fo2_cal_vector', 'X_cal_single', 'X_cal_vector', 'cal_PV_geo', 'z17', 'df_z17', 
'fo2_Z17', 'X_Z17', 'fo2_O06', 'X_O06_single', 'X_O06', 'teos', 'A18_FeO_re', 
'A18_FeO_ox', 'fo2_A18', 'X_A18_single', 'X_A18', 'par_12p5_re', 'par_12p5_ox', 
'par_25_re', 'par_25_ox', 'A3_to_cm3', 'cm3_to_A3', 'dv_12p5', 'dv_25', 'pt2v_this_study')
...
def cal_dV(result_FeO_re,result_FeO_ox,px,T,Fe_number):
    ...
    vx    = np.linspace(V0_re*0.1,V0_ox*1.5,1000)
...
W = pd.read_excel(xlsx,sheet_name='molar',nrows=4,skiprows=49,usecols=range(25),index_col=0)
...
def fo2_cal_single(P,T,PV,r,method='earth',flag=False,fit='fit3'):
    ...
    whereas here we need keep T as T.
     ...
     PV term along the geotherm is then used as input and kept in the excel prev_sum.xlsx
    """
...
def dv_12p5(p,t):
    v_re = pt2v_this_study(p,t,par_12p5_re)
    v_ox = pt2v_this_study(p,t,par_12p5_ox)
    fu = 2
    dv   = (v_ox - v_re)*A3_to_cm3/fu
    return dv

def dv_25(p,t):
    v_re = pt2v_this_study(p,t,par_25_re)
    v_ox = pt2v_this_study(p,t,par_25_ox)
    fu = 4
    dv   = (v_ox - v_re)*A3_to_cm3/fu
    return dv

def pt2v_this_study(p,t,par):
    """
    ...
    try:
        v = tl.BM4_TH_pt2v_vector_uct(par_selected,p,t,flag=False)
    except:
        ...
        if np.unique(t).size == 1:
            ...
            px   = tl.BM4_TH_vt2p_vector_uct(par_selected,vx,t[0],flag=False)
            minP = np.argmin(px)
            if minP < len(px):
                print("The min P of {} at {:2} K is around {:2.4} GPa, corresponding V is {:2.2} A^3".format(t[0], px[minP], vx[minP]))
            f    = interp1d(px[:minP],vx[:minP])
            v    = f(p)        
        else:
            print("Temperatures are varying, SLOW!!")
            for i in range(len(p)):
                px   = tl.BM4_TH_vt2p_vector_uct(par_selected,vx,t[i],flag=False)
                minP = np.argmin(px)
    #        if minP > len(px):
