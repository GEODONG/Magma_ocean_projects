...
W = pd.read_excel(xlsx,sheet_name='molar',nrows=5,usecols=range(25),index_col=0)
planets = pd.read_excel(xlsx,sheet_name='planets',nrows=3,skiprows=None,usecols = range(10),index_col=0)
G_table = pd.read_excel('db/copy_of_magma_ocean_redox_4_3.xlsx', sheet_name = 'Sheet2')
new_G_function = interp1d(G_table['Temperature'],G_table['DelG_25'])
...
def fo2_cal_single(P,T,PV,r,method='earth',flag=False,new_G = False,fit='fit3'):
    """
...
def fo2_cal_vector(P,T,dV,r,method='earth',flag=False,new_G = False,fit='fit3',PV_cal = 'old'):
    """
    ...
    return f_v(PV,T,PV,r,method=method,flag=flag,new_G = new_G,fit=fit)
...
def fo2_H22(P,T,PV,r,method='earth'):
        ...
        Y8 = -1245.09
        Y9 = -1156.86
        W_s = Y1*Si+Y2*Ti+Y3*Mg+Y4*Ca+Y5*Na+Y6*K+Y7*Ph+Y8*Si*Al+Y9*Si*Mg

    
        x = r/(1-r)
        log10fo2 = (b+c/T-dcp/R/np.log(10)*(1-T0/T-np.log(T/T0))-PV/np.log(10)+W_s/T-np.log(Fe3/Fe2))/(-a)
        return log10fo2



def fo2_H22_cal_vector(P,T,dV,r,method='earth',PV_cal = 'old'):
    """
    calculate XFe3/Xtot as a function at fixed fo2, as a function of P
    -----
    input  list
    ...
    if PV_cal == 'old':
        ...
        for i in range(len(P)):
            f = interp1d(PV_table['P(GPa)'],PV_table[PV_cal])
            PV[i]  = f(P[i])
    f_v = np.vectorize(fo2_H22,excluded=[3,4,5,6,7])
    return f_v(PV,T,PV,r,method=method)


def X_H22_single(P,T,PV,fo2,method='earth'):
    func = lambda r: fo2_H22(P,T,PV,r,method) - fo2
    try:
 #       out = opt.brentq(func, 0.0005, 0.1) # for iw
        out = opt.brentq(func, 0.0005, 0.9) # for iw
        return out
    except:
        print("r =  0.0005, func is", fo2_H22(P,T,0.0005,method) - fo2)
        print("r =  .99, func is", fo2_H22(P,T,.99,method) - fo2)    

def X_H22_cal_vector(P,T,dV,fo2,method='z17',PV_cal = 'old'):
    """
    calculate XFe3/Xtot as a function at fixed fo2, as a function of P
    -----
    input  list
    ...
    f_v = np.vectorize(X_H22_single,excluded=[4,5,6,7,8])
    return f_v(P,T,PV,fo2,method=method)
